  ...
  override fun update(world: World, deltaTime: Float) {
    world.query(Damage::class, StatusEffects::class).each { id ->
      ...
      for (incoming in damage.amounts) {
        // Already-reflected damage passes through untouched - it must land for real, and must
        // never itself trigger another reflect (that would bounce forever between two reflectors).
