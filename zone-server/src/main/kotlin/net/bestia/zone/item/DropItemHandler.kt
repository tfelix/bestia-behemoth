...
class DropItemHandler(
  ...
  override fun handle(msg: DropItemCMSG): Boolean {
    ...
    // Access the entity with a write lock, verify preconditions from ECS info and persist the
    // removal to the database immediately (critical item transaction - must not risk duplication).
    val dropPos: Vec3L? = zoneServer.withEntityWriteLock(activeEntityId) { entity ->
      ...
      val pos = entity.getOrThrow(Position::class).toVec3L()
      Vec3L(
    ...
    // 3. Spawn the ground item entity outside of the player entity's write lock.
    if (dropPos != null) {
