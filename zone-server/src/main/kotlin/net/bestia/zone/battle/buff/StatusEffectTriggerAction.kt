...
sealed interface StatusEffectTriggerAction {
  /**
   * Reacts to [incoming] damage about to be applied to [targetId] carrying [effectInstance].
   ...
   * via [world].
   */
  fun apply(world: World, effectInstance: ActiveStatusEffect, targetId: EntityId, incoming: Damage.DamageAmount): Int
   ...
   */
  data class ReflectDamage(val percent: Double) : StatusEffectTriggerAction {
    override fun apply(world: World, effectInstance: ActiveStatusEffect, targetId: EntityId, incoming: Damage.DamageAmount): Int {
