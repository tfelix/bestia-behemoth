...
/**
 * Which of one chunk column's square metres are alight.
 *
 * Rides behind the chunk payload and is dropped with it, exactly as [ChunkStaticEntitiesSMSG] is. The burn
 * *scar* left afterwards is not here - it is a ground layer, in [ChunkGroundLayersSMSG]; see either proto for
 * why what the ground is and what is happening to it travel separately.
 *
 * @property burning `chunkSize²` bits, or null when nothing here is alight - which is almost always
 */
data class ChunkGroundOverlaySMSG(
  ...
  override fun toString() =
    "ChunkGroundOverlaySMSG($chunk, ${burning?.size ?: 0}B burning)"
  ...
  override fun equals(other: Any?): Boolean {
    ...
    return chunk == other.chunk && burning.contentEquals(other.burning)
  }
