...
class FireboltTest {
  ...
  fun `damage scales with skill level`() {
    val lowLevelCast = sut.execute(ctxAt(skillLevel = 1))
    val highLevelCast = sut.execute(ctxAt(skillLevel = 10))
  ...
  fun `damage scales with intelligence`() {
    val dullCast = sut.execute(ctxAt(skillLevel = 5, intelligence = 10))
    val smartCast = sut.execute(ctxAt(skillLevel = 5, intelligence = 100))
