...
class WanderSystem : Ecs2System {
  ...
  override fun update(world: World, deltaTime: Float) {
    world.query(Velocity::class, Wander::class).each { id, vel, wander ->
      wander.step++
...
class MovementSystem : Ecs2System {
  ...
  override fun update(world: World, deltaTime: Float) {
    world.query(Position::class, Velocity::class).each { id, pos, vel ->
...
class HealthRegenSystem : Ecs2System {
  ...
  override fun update(world: World, deltaTime: Float) {
    world.query(Health::class).each { id, hp ->
      if (hp.value < hp.max) {
