...
(function(app, $, createjs, ko) {
	...
	function MousePointer(stage, displObject) {
		...
		that.mouseMoveHandler = function(event) {
			...
			that.pointer.x = tileX * config.tileSize;
			that.pointer.y = tileY * config.tileSize;
		};
	}
	...
	var Handler = {
	/**
	 * Handler is triggered if the engine is ordered to load another map.
	 */
	 ...
	 */
	/*
	 * app.Engine.renderLayer = function(layerData, tilesetSheet, tilewidth,
	 * tileheight) { for (var y = 0; y < layerData.height; y++) { for (var x =
	 * 0; x < layerData.width; x++) { // create a new Bitmap for each cell var
	 * cellBitmap = new createjs.Sprite(tilesetSheet); // layer data has single
	 * dimension array var idx = x + y * layerData.width; // tilemap data uses 1
	 * as first value, EaselJS uses 0 (sub 1 // to load correct tile)
	 * cellBitmap.gotoAndStop(layerData.data[idx] - 1); // isometrix tile
	 * positioning based on X Y order from Tiled cellBitmap.x = x * tilewidth;
	 * cellBitmap.y = y * tileheight; // add bitmap to stage
	 * app.Engine.tileContainer.addChild(cellBitmap); } } // Cache the tiles. //
	 * app.Engine.tileContainer.cache(0, 0, layerData.width * tilewidth, //
	 * layerData.height * tileheight); };
	 */
