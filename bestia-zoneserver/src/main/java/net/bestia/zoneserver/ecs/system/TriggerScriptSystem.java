...
public class TriggerScriptSystem extends EntityProcessingSystem {
	...
	protected void initialize() {
		...
		final AspectSubscriptionManager asm = world
				.getManager(AspectSubscriptionManager.class);
		collidableEntitySubscription = asm.get(Aspect.all(Bestia.class,
				Collision.class));
	}
	...
	protected void process(Entity e) {
		...
		final IntBag possibleCollisions = collidableEntitySubscription
				.getEntities();
		final Set<Integer> newCollisions = new HashSet<>();
		for (int i = 0; i < possibleCollisions.size(); i++) {
			final Entity collisionEntity = world.getEntity(possibleCollisions
					.get(i));
			final CollisionShape entityShape = collisionMapper
					.get(collisionEntity).shape;
