...
public final class SimplexNoise {
	...
	private static final Grad[] grad2 = new Grad[] { new Grad(1, 1), new Grad(-1, 1), new Grad(1, -1),
			new Grad(-1, -1), new Grad(1, 0), new Grad(-1, 0), new Grad(1, 0), new Grad(-1, 0), new Grad(0, 1),
			new Grad(0, -1), new Grad(0, 1), new Grad(0, -1) };

	private static final int[] p = new int[] { 151, 160, 137, 91, 90, 15,
	...
	private Grad[] gradP = new Grad[512];
	...
	// 2D simplex noise
	public double simplex2(int xin, int yin) {
		...
		double x1 = x0 - i1 + G2; // Offsets for middle corner in (x,y) unskewed
									// coords
		double y1 = y0 - j1 + G2;
		double x2 = x0 - 1 + 2 * G2; // Offsets for last corner in (x,y)
										// unskewed
										// coords
		double y2 = y0 - 1 + 2 * G2;
