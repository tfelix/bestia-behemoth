package net.bestia.zoneserver.game.entities;
...
public class PlayerBestiaEntity {
	private final static Logger log = LogManager.getLogger(PlayerBestiaEntity.class);
	 ...
	 */
	public PlayerBestiaEntity(PlayerBestia bestia) {
		if (bestia == null) {
	...
	/**
	 * Überprüft nach dem Hinzufügen von Exp ob ein Level Up stattgefunden hat und leitet danach alles weitere in die
	 * Wege. Wird von addExp gecalled. Darum muss addExp IMMER in PlayerBestia gecalled werden. Wird die EXP direkt ins
	 * Datenobjekt geschrieben werden evtl. Levelups nicht berücksichtigt.
	 * 
	 */
	private void checkLevelUp() {
		int neededExp = getNeededExp();
		/*
		 * if (bestia.getExp() < neededExp || bestia.getLevel() >= MAX_LEVEL) { return; }
		 */
	...
	private void calculateStatusValues() {
		 ...
		 * bestia.getEffortValue().getAtk() / 4) * bestia.getLevel() / 100 + 5; int def =
		 * (bestia.getBaseValue().getDef() * 2 + bestia.getIndividualValue().getDef() + bestia.getEffortValue().getDef()
		 * / 4) * bestia.getLevel() / 100 + 5;
		 * 
		 * int spatk = (bestia.getBaseValue().getSpAtk() * 2 + bestia.getIndividualValue().getSpAtk() +
		 * bestia.getEffortValue().getSpAtk() / 4) * bestia.getLevel() / 100 + 5; int spdef =
		 * (bestia.getBaseValue().getSpDef() * 2 + bestia.getIndividualValue().getSpDef() +
		 * bestia.getEffortValue().getSpDef() / 4) * bestia.getLevel() / 100 + 5; int spd =
		 * (bestia.getBaseValue().getSpd() * 2 + bestia.getIndividualValue().getSpd() + bestia.getEffortValue().getSpd()
		 * / 4) * bestia.getLevel() / 100 + 5;
		 */
		// TODO HP und Mana passen nicht ins Statuspunkt konzept da sie verändelrich sind.
		// eigene klasse?
		/*
		 * int hp = bestia.getBaseValue().getMaxHp() $hp = floor(($this->data->get('b_hp')*2 + $this->data->get('iv_hp')
		 * + floor($this->data->get('ev_hp')/4) ) * $this->data->get('level')/100 + 10 + $this->data->get('level') );
		 * $mana = floor(($this->data->get('b_mana')*2 + $this->data->get('iv_mana') +
		 * floor($this->data->get('ev_mana')/4) ) * $this->data->get('level')/100 + 10 + $this->data->get('level') * 2);
		 */
		// StatusPoints points = bestia.getStatusPoints();
		/*
		 * points.setAtk(atk); points.setDef(def); points.setSpAtk(spatk); points.setSpDef(spdef); points.setSpd(spd);
		 */
