...
import net.bestia.zoneserver.connection.LogoutService
...
class ClientConnectionActorEx(
	...
	private fun checkMessageEnvelope(msg: ClientToMessageEnvelope) {
		...
		when (content) {
			is ClientConnectionStatusMessage -> handleNewClientConnection(content)
			else -> sendMessageToClient(msg.content)
		}
	}

	private fun handleNewClientConnection(msg: ClientConnectionStatusMessage) {
	 ...
	 */
	private fun initClientConnection(msg: ClientConnectionStatusMessage) {
